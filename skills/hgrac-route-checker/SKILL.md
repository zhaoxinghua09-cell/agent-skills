...
- 规则源：`《人类遗传资源管理条例》(国务院令717号)+实施细则(2023-07-01施行) 第8-22条`（以官方最新文本为准）。

## 三、用法
```bash
python hgrac-route-checker.py --mtow 5          # 按参数判定
python hgrac-route-checker.py --demo            # 跑内置冒烟案例
```
参数：`action / foreign_involved / scale`

## 四、真机输出（--demo 节选）
```json
{"tool":"hgrac-route-checker","result":{"grade":"示例等级"},"rc":0}
```
