...
- 规则源：`TH-FIN-002 母版（FDA SaMD 分级思维平移金融）+ 金融AI监管导向`（以官方最新文本为准）。

## 三、用法
```bash
python fin-ai-classifier.py --mtow 5          # 按参数判定
python fin-ai-classifier.py --demo            # 跑内置冒烟案例
```
参数：`autonomy / impact / target`

## 四、真机输出（--demo 节选）
```json
{"tool":"fin-ai-classifier","result":{"grade":"示例等级"},"rc":0}
```
