...
- 规则源：`NMPA 进口医疗器械注册规定 + 出口销售证明办理 + FDA/EU MDR`，请以官方正式文本核对。
...
- 规则源：`NMPA 进口医疗器械注册规定 + 出口销售证明办理 + FDA/EU MDR`（以官方最新文本为准）。

## 三、用法
```bash
python import-export-router.py --device_class III --device_type implant   # 按参数判定
python import-export-router.py --demo                                    # 跑内置冒烟案例
```
参数：`direction / is_registered / dest_region`

## 四、真机输出（--demo 节选）
```json
{"tool":"import-export-router","result":{"grade":"示例等级"},"rc":0}
```
