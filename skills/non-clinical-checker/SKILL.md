...
## 三、用法
```bash
python non-clinical-checker.py --device_class III --device_type implant   # 按参数判定
python non-clinical-checker.py --demo                                    # 跑内置冒烟案例
```
参数：`device_class / device_type / contact`

## 四、真机输出（--demo 节选）
```json
{"tool":"non-clinical-checker","result":{"grade":"示例等级"},"rc":0}
```
